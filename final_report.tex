...
This preliminary DRL result is encouraging but should be interpreted cautiously. The PPO agent delivers much stronger return and Sharpe ratio than both the original and enhanced rule-based allocators on the strict out-of-sample 2021--2024 test window, but its tail risk remains materially worse than the enhanced GARCH-CVaR baseline. In particular, the DRL test CVaR of 7.85\% is substantially higher than the enhanced baseline's 5.15\%, even though the DRL Sharpe ratio is higher.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{outputs/drl_equity_curves.png}
\caption{DRL versus baseline equity curves on the out-of-sample test window}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{outputs/drl_test_weights.png}
\caption{PPO-based DRL test-period portfolio weights across SPY, TLT, and GLD}
\end{figure}
